...
export default function ChatNpcScreen({
  ...
  useEffect(() => {
    ...
    if (!el) return; // 여기서 null 체크
    requestAnimationFrame(() => {
      el.scrollTop = el.scrollHeight; // 스크롤 바닥으로
    });
  }, [messages, isChatOpen]); // messages가 바뀔 때마다 실행
  ...
  if (isChatOpen) {
    ...
    else if (worldKey === 'sacrifice') return (
      <div
        className={`
