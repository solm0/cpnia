...
import { checkCollision } from "@/app/components/maps/player/checkCollision";
...
import { findAncestorWithId, getNearbyMeshes, hasAncestorWithId } from "@/app/components/maps/sacrifice/Player";
...
export default function Player({
  ...
  const { yaw } = useFollowCam(
    body,
    [0, config?.camYPos ?? 0, config?.camZPos ?? 0],
    [degToRad(5), degToRad(config?.camYRot ?? 90), 0],
    pressedKeys.current,
    gamepad.current,
    { zoomMin: 5, zoomMax: 20, pitchMin: -0.2, pitchMax: 0.2 }
  );
